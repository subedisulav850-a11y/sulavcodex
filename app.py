...
# Protobuf imports (user must place these files in the same directory)
try:
...
ALL_BADGE_NAMES = list(PRIME_FILES.keys()) + list(CUSTOM_BADGE_FILES.keys())
...
ALL_FRAME_NAMES = list(CUSTOM_FRAME_FILES.keys())
...
# Gallery Items Configuration (default – can be changed via API)
ITEMS = {
    "item1": 212000000,
    "item2": 203000000,
    "item3": 212000000,
...
def get_config(key, default=None):
    ...
    if CACHED_CONFIG is None:
        # If not cached, try sync fetch (this runs in thread pool)
        import asyncio
        try:
            ...
            if loop.is_running():
                ...
                loop.run_until_complete(fetch_update_config_async())
        except:
            pass
    if CACHED_CONFIG is None:
        return default
...
def set_gallery_items(jwt_token, items_dict=None):
    # Placeholder – actual implementation from the Flask app should be copied here.
...
# ===== Existing endpoints (from previous final version) =====
@app.get("/player-info")
...
# ===== Updated Duo Endpoint (fully working) =====
@app.get("/duo")
async def get_duo(
    uid: str = Query(..., description="Player UID"),
    password: str = Query("GUEST_PASSWORD", description="Guest password (optional)"),
    info: Optional[str] = Query(None, description="If present, returns raw data")
):
    """
    Get Dynamic Duo (Special Friend) information.
    Returns formatted human-readable data by default.
    Pass 'info=1' to get raw parsed data.
    """
    # 1. Generate JWT (cached)
    ...
    # 2. Build request payload (protobuf: field 1 = UID as varint)
    def build_duo_request(uid: str) -> bytes:
        ...
        while True:
            ...
            if not n:
                break
        # Protobuf message: field number 1, wire type 0 (varint)
        payload = b"\x08" + bytes(varint)
        # Encrypt with AES (same key/iv as CLI)
        cipher = AES.new(AES_KEY, AES.MODE_CBC, AES_IV)
    ...
    if duo_pb2 is None:
        raise HTTPException(500, "Duo protobuf module not loaded")
    ...
    try:
        ...
        if not msg.HasField("duo_info"):
            return JSONResponse(content={
                "success": True,
                "has_duo": False,
                "message": "No Dynamic Duo found for this player"
            })
        duo = msg.duo_info
    except Exception as e:
