...
# ===== Duo Endpoint =====
@app.get("/duo")
async def get_duo(uid: str = Query(...), password: Optional[str] = Query("GUEST_PASSWORD")):
    if duo_pb2 is None:
        raise HTTPException(500, "Duo protobuf not loaded")
    jwt_token = None
    try:
        res = generate_jwt_sync(uid, password, "AUTO")
        if res.get("success"):
            jwt_token = res.get("jwt_token")
    except:
        pass
    if not jwt_token:
        raise HTTPException(401, "Could not obtain JWT")
    try:
        headers = {
        ...
        # Build payload – simplified for now; in production you need the correct protobuf.
        dummy_duo = {
        ...
        return JSONResponse(content={"success": True, "duo_data": dummy_duo})
    except Exception as e:
